...
export class GiftRegistryPDF {
  ...
  constructor(options) {
    ...
    // 确保后续使用的字体URL存在默认值
    this.options.amountFontUrl =
      this.options.amountFontUrl || this.options.formalFontUrl;
    this.options.coverFontUrl =
      this.options.coverFontUrl || this.options.formalFontUrl;
  ...
  async _addGiftsPages(
    pdfDoc,
    fonts,
    { validData: data, mainContentTotalPages },
  ) {
    ...
    for (let p = 0; p < mainContentTotalPages; p++) {
      ...
      pageData.forEach((item, i) => {
        ...
        this._drawText(
          page,
          String.fromCharCode(0x00a5) + item.amount,
          fonts.formalFont,
          {
            x: colX,
            y: margin.bottom + 5,
  ...
  async _addSummaryAppendix(pdfDoc, fonts, processedData) {
    if (!processedData || Object.keys(processedData.summary).length === 0)
      return;
    ...
    const summaryData = Object.entries(processedData.summary).map(
      ([type, values]) => ({
        method: type,
        count: `${values.count} 人`,
        amount: this._formatRMB(values.total),
      }),
    );
    ...
    if (this.options.partIndex && this.options.totalParts) {
      tableData.push({
        method: "本部分总计",
        count: `${partTotalItems} 人`,
        amount: this._formatRMB(partTotalAmount),
      });
      tableData.push({
        method: "事项总金额",
        count: `${this.options.grandTotalGivers || 0} 人`,
    ...
    page.drawLine({
      start: { x: margin.left, y: cursorY - headerHeight },
      end: { x: margin.left + tableWidth, y: cursorY - headerHeight },
      color: this.colors.red,
      thickness: 0.8,
    });
    ...
    tableData.forEach((rowData) => {
      ...
      page.drawLine({
        ...
        end: { x: margin.left + tableWidth, y: cursorY - rowHeight },
        color: this.colors.red,
        thickness: 0.8,
      });
      cursorY -= rowHeight;
    ...
    page.drawLine({
      start: { x: margin.left, y: tableTopY },
      end: { x: margin.left + tableWidth, y: tableTopY },
      color: this.colors.red,
      thickness: 1.2,
    });
    page.drawLine({
      start: { x: margin.left, y: tableBottomY },
      end: { x: margin.left + tableWidth, y: tableBottomY },
      color: this.colors.red,
      thickness: 1.2,
    });
    page.drawLine({
      start: { x: margin.left, y: tableTopY },
      end: { x: margin.left, y: tableBottomY },
      color: this.colors.red,
      thickness: 1.2,
    });
    page.drawLine({
      start: { x: margin.left + tableWidth, y: tableTopY },
      end: { x: margin.left + tableWidth, y: tableBottomY },
      color: this.colors.red,
      thickness: 1.2,
    });
    ...
    if (this.options.recorder || this.options.subtitle) {
      const recorderText =
        this.options.recorder ? `记账人:  ${this.options.recorder}` : "";
      const recorderTextWidth = mainFont.widthOfTextAtSize(recorderText, 18);
      const dateTextWidth =
        this.options.subtitle ?
          mainFont.widthOfTextAtSize(this.options.subtitle, 18)
        : 0;
      ...
      if (this.options.subtitle) {
        ...
        page.drawText(this.options.subtitle, {
          x: centeredX,
          y: cursorY,
          font: mainFont,
          size: 18,
  ...
  async _addRemarkAppendix(pdfDoc, fonts, processedData) {
    ...
    for (const row of remarks) {
      const { lines, height } = this._wrapText(
        row.remark,
        mainFont,
        11,
        colWidths[2] - 12,
    ...
    appendixPages.forEach((p, idx) => {
      ...
      p.drawLine({
        start: { x: margin.left, y: topY },
        end: { x: margin.left + tableWidth, y: topY },
        color: this.colors.red,
        thickness: 1.2,
      });
      p.drawLine({
        start: { x: margin.left, y: topY },
        end: { x: margin.left, y: bottomY },
        color: this.colors.red,
        thickness: 1.2,
      });
      p.drawLine({
        start: { x: margin.left + tableWidth, y: topY },
        end: { x: margin.left + tableWidth, y: bottomY },
        color: this.colors.red,
        thickness: 1.2,
      });
      p.drawLine({
        start: { x: margin.left, y: bottomY },
        end: { x: margin.left + tableWidth, y: bottomY },
        color: this.colors.red,
        thickness: 1.2,
      });
  ...
  _drawText(page, text, font, options) {
    ...
    if (!isVertical) {
      ...
      while (
        ...
        font.widthOfTextAtSize(text, fontSize) > cellWidth * 0.9
      ) {
        fontSize -= 0.5;
      }
    ...
    for (let c = 0; c < finalColCount; c++) {
      const colChars = chars.slice(
        c * charsPerColForDrawing,
        (c + 1) * charsPerColForDrawing,
      );
      ...
      colChars.forEach((char, r) => {
        ...
        page.drawText(char, {
          x:
            blockLeft +
            c * (finalFontSize + letterSpacing) +
            (finalFontSize - charWidth) / 2,
  ...
  async generate(data, newOptions = null) {
    ...
    const fonts = {
      mainFont: await pdfDoc.embedFont(this.resources.fontBytes, {
        subset: true,
      }),
      giftLabelFont:
        this.resources.giftLabelFontBytes ?
          await pdfDoc.embedFont(this.resources.giftLabelFontBytes, {
            subset: true,
          })
        : null,
      formalFont:
        this.resources.formalFontBytes ?
          await pdfDoc.embedFont(this.resources.formalFontBytes, {
            subset: true,
          })
        : null,
      amountFont:
        this.resources.amountFontBytes ?
          await pdfDoc.embedFont(this.resources.amountFontBytes, {
            subset: true,
          })
        : null,
      coverFont:
        this.resources.coverFontBytes ?
          await pdfDoc.embedFont(this.resources.coverFontBytes, {
            subset: true,
          })
        : null,
    };
    ...
    if (this.options.printCover && this.resources.coverImageBytes) {
      ...
      await this._drawImageOnPage(
        pdfDoc,
        coverPage,
        this.resources.coverImageBytes,
      );
      ...
      if (this.options.showCoverTitle && this.options.title) {
        const titleWidth = coverFont.widthOfTextAtSize(
          this.options.title,
          coverFontSize,
        );
      ...
      if (this.options.showCoverTitle && this.options.subtitle) {
        const subtitleWidth = coverFont.widthOfTextAtSize(
          this.options.subtitle,
          coverFontSize,
        );
    ...
    if (
      this.options.printEndPage !== false &&
      this.resources.backCoverImageBytes
    ) {
